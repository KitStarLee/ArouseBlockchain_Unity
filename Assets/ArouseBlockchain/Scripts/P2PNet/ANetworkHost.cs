...
namespace ArouseBlockchain.P2PNet
{
	...
	[AddComponentMenu("SuperNet/ANetworkHost")]
	public sealed class ANetworkHost : MonoBehaviour, IHostListener, IPeerListener {
        ...
        public readonly Dictionary<IPEndPoint, NodePeerNet> PeersConnected;

		private readonly Dictionary<Peer, IPeerListener> Listeners;
		...
		public ANetworkHost() {
			...
			PeersConnected = new Dictionary<IPEndPoint, NodePeerNet>();
			Listeners = new Dictionary<Peer, IPeerListener>();
        ...
        //    Debug.Log(string.Format("[02-1Host 关闭]"));
        //    // Dispose host
        //    host?.Dispose();
        //}
		...
		public bool SetNodePeerState(IPEndPoint address,NodePeer nodePeer)
		{
            if (PeersConnected.ContainsKey(address))
            {
                PeersConnected[address].NodePeer = nodePeer;
                return true;
            }
		...
		public bool SetPeerListener(IPEndPoint remote, IPeerListener listener) {
			try {
				Lock.EnterWriteLock();
				if (PeersConnected.ContainsKey(remote)) {
					Debug.Log("01 添加事件： " + remote.ToString()) ;
					Listeners[PeersConnected[remote].Peer] = listener;
					return true;
				} else {
					return false;
				}
		...
		public void Dispose() {
			...
			if (host != null && NetworkManager.LogEvents) {
				ALog.Log(string.Format(
					...
					host.BindAddress, PeersConnected.Count
				));
			}
			ALog.Log("Net Host 释放自己 02");
			// Dispose
		...
		public void Shutdown() {
			...
			try {
				...
				host = Host;
				Host = null;
                if (PeersConnected != null && PeersConnected.Count > 0)
                {
                    foreach (var remote in PeersConnected.Keys)
                    {
                        PeersConnected[remote].Peer?.DisconnectAsync();
                      //  PeersConnected[remote].Peer?.Dispose();
                    }
		...
		public void SendAll(SuperNet.Netcode.Transport.IMessage message, params Peer[] exclude) {
			try {
				Lock.EnterReadLock();
		...
		public Peer Connect(IPEndPoint remote, IPeerListener listener = null, IWritable message = null, bool rethrow = false) {
			...
			try {
				...
				if (PeersConnected.Count >= MaxConnections) {
					if (NetworkManager.LogExceptions) {
						Debug.LogWarning(string.Format(
				...
				bool found = PeersConnected.TryGetValue(remote, out NodePeerNet existing);
				if (found && existing.Peer.Connected) {
					if (NetworkManager.LogExceptions) {
						Debug.LogWarning(string.Format("[SuperNet] [Host] Already connected to '{0}'.", remote));
					}
					return existing.Peer;
				} else if (found && existing.Peer.Connecting) {
					if (NetworkManager.LogExceptions) {
				...
				if (existing != null) existing.Peer.Dispose();
				...
				// Save peer & listener
				PeersConnected[remote] = new NodePeerNet() { Peer = peer};
				PeersConnected[remote].NodePeer.is_online = true;

                Listeners[peer] = listener;
				//Debug.Log("02 事件添加成功 ：" + remote.ToString());

                // Replace peer array
                Peer[] peers = new Peer[PeersConnected.Count];
                PeersConnected.Select(x => x.Value.Peer).ToArray().CopyTo(peers,0);
                //PeersConnected.Values.CopyTo(netpeers, 0);
		...
		void IHostListener.OnHostReceiveRequest(ConnectionRequest request, Reader message) {
			...
			try {
				...
				if (peer == null) {
					...
					bool found = PeersConnected.TryGetValue(request.Remote, out NodePeerNet existing);
					if (found && (existing.Peer.Connected || existing.Peer.Connecting)) {
						if (NetworkManager.LogExceptions) {
							Debug.LogWarning(string.Format(
					...
					// Replace listener
                    //Debug.Log("03 事件修改： " + peer.Remote.ToString());
				...
				// Save peer
				PeersConnected[request.Remote] = new NodePeerNet() { Peer = peer };
				PeersConnected[request.Remote].NodePeer.is_online = true;
		...
		void IPeerListener.OnPeerConnect(Peer peer) {
			...
			try {
				...
				if(!PeersConnected.ContainsKey(peer.Remote))
				{
					PeersConnected[peer.Remote] = new NodePeerNet();
                }
				PeersConnected[peer.Remote].Peer = peer;
				PeersConnected[peer.Remote].NodePeer.is_online = true;
			...
			// Invoke listener
			try {
                //Debug.Log("第二次----" + peer.Remote.ToString());
		...
		void IPeerListener.OnPeerDisconnect(Peer peer, Reader message, DisconnectReason reason, Exception exception) {
			...
			try {
				...
				PeersConnected.Remove(peer.Remote);
		...
		void IHostListener.OnHostShutdown() {
            ...
            ALog.Log("！！Net Host OnHostShutdown 01");
		...
		void IHostListener.OnHostException(IPEndPoint remote, Exception exception) {
            ...
            ALog.Log("！！Net Host OnHostException 01");
		...
		private IPeerListener FindListener(Peer peer) {
			try {
				Lock.EnterReadLock();
				Listeners.TryGetValue(peer, out IPeerListener listener);
				//Debug.Log("listener is null :" + (listener == null));
